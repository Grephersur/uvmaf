...
$Config.ReportsPath = Join-Path $Config.BasePath "Reports_RC1"
$Config.Folders = @{
...
# In-code defaults (for switches & non-mandatory params)
$Config.Defaults = @{
    SkipValidation = $false
    GenerateDashboard = $true
    ReportTypes = @("Workstations", "MS Servers", "Non-MS Servers")
    FastMode = $true
    NoProgress = $false
    SkipFormatting = $true
    ValidationMaxFiles = 0
    CreateMissingScopes = $true
    ForceReprocess = $true
    ValidationDegree = 3 # New: default for the ValidationDegree param
}

# Respect defaults if args not supplied
if (-not $PSBoundParameters.ContainsKey('SkipValidation')) { if ($Config.Defaults.SkipValidation) { $SkipValidation = $true } }
if (-not $PSBoundParameters.ContainsKey('GenerateDashboard')) { if ($Config.Defaults.GenerateDashboard) { $GenerateDashboard = $true } }
if (-not $PSBoundParameters.ContainsKey('ReportTypes')) { $ReportTypes = $Config.Defaults.ReportTypes }
if (-not $PSBoundParameters.ContainsKey('FastMode')) { if ($Config.Defaults.FastMode) { $FastMode = $true } }
if (-not $PSBoundParameters.ContainsKey('NoProgress')) { if ($Config.Defaults.NoProgress) { $NoProgress = $true } }
if (-not $PSBoundParameters.ContainsKey('SkipFormatting')) { if ($Config.Defaults.SkipFormatting) { $SkipFormatting = $true } }
if (-not $PSBoundParameters.ContainsKey('ValidationMaxFiles')) { $ValidationMaxFiles = $Config.Defaults.ValidationMaxFiles }
if (-not $PSBoundParameters.ContainsKey('CreateMissingScopes')) { if ($Config.Defaults.CreateMissingScopes) { $CreateMissingScopes = $true } }
if (-not $PSBoundParameters.ContainsKey('ForceReprocess')) { if ($Config.Defaults.ForceReprocess) { $ForceReprocess = $true } }
if (-not $PSBoundParameters.ContainsKey('ValidationDegree')) { $ValidationDegree = $Config.Defaults.ValidationDegree }
...
function Get-LatestReportFile { param([string]$Folder,[string[]]$Patterns)
    ...
    $files = @()
    # CORRECTED: Added a wildcard to the path to ensure -Include works correctly
    $files += Get-ChildItem -Path (Join-Path $Folder '*') -File -Include $Patterns -ErrorAction SilentlyContinue
...
function Import-TenableCSV { param([Parameter(Mandatory=$true)][string]$CsvPath)
    ...
    try{
        ...
        $columnsToKeep = New-Object System.Collections.Generic.HashSet[string]
        $columnsToKeep.Add('asset.name')
        $columnsToKeep.Add('asset.fqdn')
        $columnsToKeep.Add('asset.hostname')
        $columnsToKeep.Add('asset.netbios_name')
        $columnsToKeep.Add('asset.display_ipv4_address')
        $columnsToKeep.Add('definition.id')
        $columnsToKeep.Add('definition.name')
        $columnsToKeep.Add('severity')
        $columnsToKeep.Add('last_seen')
        $columnsToKeep.Add('state')
        $columnsToKeep.Add('definition.solution')
        $columnsToKeep.Add('output')
        $columnsToKeep.Add('ipv4')
        $columnsToKeep.Add('host-ip')
        $columnsToKeep.Add('plugin.id')
        $columnsToKeep.Add('plugin.name')

        $data = $data | ForEach-Object {
            $newObj = [pscustomobject]@{}
            foreach ($prop in $_.PSObject.Properties) {
                if ($columnsToKeep.Contains($prop.Name)) {
...
# REFACTORED: Now uses a temporary file for atomic updates.
function Update-WeeklyReport {
    ...
    $tempPath = [System.IO.Path]::ChangeExtension($tempPath, 'xlsx')

    try{
        Write-Log "Updating ${ReportType}: $ReportPath"

        # Read headers from the template file
        $pkgIn = Open-ExcelPackageWithRetry -Path $ReportPath
        $wsIn = $pkgIn.Workbook.Worksheets['Vulnerability Data']
        if(-not $wsIn -or -not $wsIn.Dimension){ throw "Worksheet 'Vulnerability Data' has no headers/data" }
        $headersIn = @()
        for($c=1;$c -le $wsIn.Dimension.End.Column;$c++){ $h=($wsIn.Cells[1,$c].Text).Trim(); if($h){ $headersIn+=$h } }
        $colMap = @{}; for($c=1;$c -le $wsIn.Dimension.End.Column;$c++){ $h=($wsIn.Cells[1,$c].Text).Trim(); if($h){ $colMap[$h]=$c } }
        
        $must=@('Severity','Plugin Name','Plugin ID','Status','Parent Ticket','Mitigation Plan')
        $miss=$must | Where-Object { $_ -notin $headersIn }
        if($miss){ throw "Template missing required columns: $($miss -join ', ')" }

        $headers = (Insert-HeaderAfter -Headers $headersIn -NewHeader 'Latest CHG Date' -AfterHeader 'Mitigation Plan')
        ...
        # Import data and process in memory
        $rows = @(Import-Excel -ExcelPackage $pkgIn -WorksheetName 'Vulnerability Data')
        ...
        if(-not $FastMode -and $Config.Performance.EnableAutoSize -and -not $SkipFormatting){
            $sel | Export-Excel -Path $tempPath -WorksheetName 'Vulnerability Data' -ClearSheet -AutoFilter -FreezeTopRow -AutoSize -NoNumberConversion:$true
        } else {
            $sel | Export-Excel -Path $tempPath -WorksheetName 'Vulnerability Data' -ClearSheet -AutoFilter -FreezeTopRow -NoNumberConversion:$true
        }

        # Rebuild tabs + set column formats
        $pkgOut = Open-ExcelPackageWithRetry -Path $tempPath
        ...
        if($pkgIn){ try{ $pkgIn.Dispose() }catch{} }
...
function Run-ScopeValidations { param([array]$VulnData)
    ...
    if(-not $Config.Performance.ValidationParallel){
        ...
        # CORRECTED: This is the proper way to add a module to the InitialSessionState for PS 5.1
        $null = $iss.ImportPSModule(@('ImportExcel'))
        ...
        foreach($sf in $files){
            ...
            $jobs += [pscustomobject]@{ PS=$ps; Handle=$handle; File=$sf.FullName; Name=$sf.Name; Done = $false }
        }
        ...
        while($completed -lt $jobs.Count){
            ...
            foreach($j in ($jobs | Where-Object { $_.Handle.IsCompleted -and !$_.Done })){
                try { 
...
function Run-SingleScopeValidation { param([string]$ScopeFile,[hashtable]$VulnLookup)
    ...
    try{
        $scopeData = @(Import-Excel -Path $ScopeFile)
        ...
        if(-not $FastMode -and $Config.Performance.EnableAutoSize -and -not $SkipFormatting){ $sorted | Export-Excel -Path $outPath -WorksheetName 'Validation Data' -AutoFilter -AutoSize -NoNumberConversion
        } else { $sorted | Export-Excel -Path $outPath -WorksheetName 'Validation Data' -AutoFilter -NoNumberConversion }

        try{
            $p = Open-ExcelPackageWithRetry -Path $outPath
            $ws=$p.Workbook.Worksheets['Validation Data']
...
function Main {
    ...
    try{
        ...
        Update-Progress -Id $PROG_MAIN -Activity "Archiving CSV..." -PercentComplete 92
